...
- Keep durable procedures and decisions in `checklist/`, task decomposition in the active-state directories under `kanban/`, and reusable findings in `shared-context/`.
- Keep every unfinished task in exactly one active-state directory: `discussion/`, `planning/`, or `executable/`.
- Advance tasks only in this order: `discussion/` → `planning/` → `executable/` → `done/`.
